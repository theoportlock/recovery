...
# NEED TO SORT OUT THE HEALTHY SECOND SAMPLE - 2yr not 15 month
...
taxo = taxo.T.join(samplesheet.reset_index().set_index('Seq_ID')['TimeID'], how='inner').set_index('TimeID').T
f.save(taxo, 'metaphlan')
taxo = taxo.T
...
allpathwaystaxo = pd.concat(output, axis=1)
allpathwaystaxo = f.norm(allpathwaystaxo)
