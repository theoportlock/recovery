...
extra = pd.read_excel("../data/LEAP Anthro Recovery status files including samples_batch4_5.xlsx", index_col=0)
...
# Merge feed and recovery status
meta = meta.join(extra[['Feeds by Randomization','Recovery']])
meta.loc[:,"Recovery"] = meta.Recovery.replace({True:'Recovered',False:'No recovery'})
...
# Rename Recovery
meta.loc[:, 'Recovery'] = meta.Recovery.fillna('Healthy')
#meta.loc[:, 'Recovery'] = meta.Recovery.fillna('Recovered')

# Rename Feeds
meta.loc[:, 'Feeds by Randomization'] = meta['Feeds by Randomization'].fillna('Healthy')
#meta.loc[:, 'Feeds by Randomization'] = meta['Feeds by Randomization'].replace({'Local RUSF (A)':0,'ERUSF (B)':1})
...
# save timemeta
timepoints = ['000', '012', '052', '104']
timemeta = pd.DataFrame(index=pd.concat([meta.index.to_series() + i for i in timepoints]))
timemeta['ID'] = timemeta.index.str[:-3]
timemeta['timepoint'] = timemeta.index.str[-3:].astype(int)
timemeta.index = timemeta.index.set_names('timeID')
f.save(timemeta, 'timemeta')
