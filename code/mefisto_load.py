...
def main():
    ...
    for f in args.omics:
        ...
        df_long = df.reset_index().melt(
            id_vars=["index", "subjectID", "timepoint"] + [c for c in timemeta.columns if c not in ["subjectID", "timepoint"]],
            var_name="feature",
            value_name="value"
        )
    ...
    # Rename columns
    final = final.rename(columns={"subjectID": "group"})
    final = final.rename(columns={"index": "sample"})
    print(final)
